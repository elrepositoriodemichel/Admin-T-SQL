...
| `dm_exec_connections` | Conexión física | `INNER` | Toda sesión de usuario activa tiene conexión — proporciona `most_recent_sql_handle` y `client_net_address`. **Nota:** Las sesiones de sistema pueden no tener entrada en esta vista; si se requiere incluirlas, usar `LEFT JOIN`. |
