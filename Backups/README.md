...
> ⚠️ **Importante:** La opción `COPY_ONLY` **no tiene efecto** cuando se combina con `DIFFERENTIAL`. Un backup diferencial con `COPY_ONLY` se comporta como un backup diferencial normal y rompe la cadena de backups.
...
| Opción en SSMS | Equivalente T-SQL | Cuándo usarla |
|---|---|---|
| ***Verify backup when finished*** | `RESTORE VERIFYONLY` (posterior) | Comprueba que el backup es legible y realiza comprobaciones adicionales sobre los datos para detectar errores, acercándose lo máximo posible a una operación de restauración real. **Sin embargo, no es un sustituto de una restauración de prueba completa.** |
...
La operación `BACKUP TO URL` de SQL Server tiene un límite de aproximadamente **200 GB por archivo (`URL`)**. Para bases de datos que superen este tamaño, es necesario utilizar el **striping** (distribuir el backup en múltiples archivos URL), lo que permite alcanzar un tamaño máximo de backup de **~12.8 TB** (64 archivos * ~200 GB). Para bases de datos aún más grandes, se deben considerar otras estrategias como backups en disco local o el uso de soluciones de terceros.
...
>  ⚠️  Para restaurar un backup en stripe hay que especificar **todos los blobs** en el mismo orden en que se crearon. El límite técnico de archivos en un Backup Stripe de SQL Server es de 64.
