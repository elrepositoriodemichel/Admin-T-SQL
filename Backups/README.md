...
Guía de referencia que empareja las opciones de backup disponibles en el asistente gráfico de SSMS con su equivalente en T-SQL. Al final se incluye una recomendación de validación con **dbatools** y el uso de los scripts de Ola Hallengren.
...
| **Copy-only backup** | `WITH COPY_ONLY` | No rompe la cadena de backups — **útil para copias puntuales sin afectar al plan de mantenimiento** |
...
| **Files and filegroups** | `BACKUP DATABASE [nombre] FILE = 'nombre_fichero'` o `FILEGROUP = 'nombre_filegroup'` | Solo útil en bases de datos muy grandes con múltiples filegroups |

### Destino

| Opción en SSMS | Equivalente T-SQL | Notas |
|---|---|---|
| **Back up to: Disk** | `TO DISK = 'ruta\fichero.bak'` | Opción habitual en entornos modernos |
| **Back up to: URL** | `TO URL = 'https://...'` | Para Azure Blob Storage |
| **Múltiples destinos** | `TO DISK = '...', DISK = '...'` | El backup se distribuye en franjas entre los ficheros — mejora el rendimiento de escritura |
...
> Esta sección se centra en backups a **disco**. Las opciones de cinta (`Tape drive`) quedan fuera del alcance de esta guía.

### Overwrite media — Comportamiento sobre el fichero destino

Esta sección controla qué ocurre con el fichero `.bak` si ya existe o ya contiene backups previos.
...
| **Check media set name and backup set expiration** | `WITH MEDIANAME = '...'` | Valida que el nombre del media set coincide y que ningún backup set contenido ha expirado antes de permitir la escritura.|

> 💡 Para backups a ficheros individuales y rotación diaria lo más habitual es usar `WITH INIT` o `WITH FORMAT` para evitar que los ficheros crezcan indefinidamente acumulando backups.

### Reliability — Fiabilidad

| Opción en SSMS | Equivalente T-SQL | Cuándo usarla |
|---|---|---|
| **Verify backup when finished** | `WITH VERIFY` | Comprueba que el backup es legible una vez escrito. **No valida que los datos sean recuperables**, solo que el fichero no está corrupto. Recomendado siempre. |
...
> ⚠️ `WITH VERIFY` en VLDBs tiene un impacto real y medible:
El proceso de verificación lee secuencialmente todo el fichero de backup para validar su integridad estructural. En una VLDB esto significa:
>- **I/O adicional significativo:** SQL Server tiene que leer el fichero .bak completo una segunda vez después de escribirlo. En un backup de 5 TB esto supone 5 TB adicionales de lectura sobre el almacenamiento de backup.
>- **Tiempo adicional:** puede incrementar el tiempo total del proceso de backup entre un 50% y un 100% dependiendo de la velocidad del almacenamiento destino.
>- **No bloquea la base de datos:** la verificación opera sobre el fichero ya escrito, no sobre la base de datos, por lo que no hay impacto en disponibilidad.
>
>La recomendación de la comunidad para VLDBs es precisamente la que ya tenemos documentada — usar `WITH CHECKSUM` durante la escritura del backup en lugar de o además de `WITH VERIFY`. El checksum valida cada página en el momento de escribirla sin necesidad de una segunda pasada de lectura completa, siendo mucho más eficiente en términos de I/O total.
...
| **Back up the tail of the log** | Solo con `BACKUP LOG` | `WITH NORECOVERY` — deja la BD en estado de restauración para continuar aplicando backups |

> ℹ️ Con **Recovery Model = SIMPLE** ambas opciones aparecen deshabilitadas, ya que SQL Server gestiona el log automáticamente y no permite backups de log.
...
| Opción en SSMS | Equivalente T-SQL | Notas |
|---|---|---|
| **After N days** | `WITH RETAINDAYS = N` | SQL Server no permitirá sobrescribir el backup hasta que pasen N días. `0` = sin expiración. |
...
| **Compress backup** | `WITH COMPRESSION` | Disponible desde SQL Server 2008 en ediciones **Standard y superior** |
...
> ⚠️ Cuando TDE está activo, la compresión de backup opera sobre páginas ya cifradas, lo que hace que el ratio de compresión sea cercano a 0% en la mayoría de los casos, pudiendo incluso generar un fichero ligeramente mayor que el original por el overhead de metadatos de compresión que se añade sin conseguir reducción alguna.
...
>- En VLDBs con TDE activar compresión de backup es esencialmente desperdiciar ciclos de CPU

### Cifrado

| Opción en SSMS | Equivalente T-SQL | Disponibilidad |
|---|---|---|
| **Encrypt backup** | `WITH ENCRYPTION (ALGORITHM = AES_256, SERVER CERTIFICATE = ...)` | Requiere **`WITH FORMAT`** en Media Options — no disponible con `NOINIT` o `INIT` |
...
CREATE CERTIFICATE BackupCert
    WITH SUBJECT = 'Certificado para cifrado de backups',
...
### Estrategia habitual: FULL semanal + DIFFERENTIAL cada 12 horas + LOG cada 10 minutos

Esta estrategia es la más común en entornos de producción con Recovery Model `FULL` donde se requiere mínima pérdida de datos (RPO bajo) y tiempos de restauración razonables. El FULL semanal minimiza el impacto en el servidor, el DIFF cada 12 horas reduce la cantidad de LOGs a aplicar en una restauración, y el LOG cada 10 minutos acota la pérdida máxima de datos a ese intervalo.
...
> ⚠️ **Con TDE activo:** la opción `COMPRESSION` genera overhead de CPU sin beneficio real en reducción de tamaño al operar sobre páginas ya cifradas. Valorar sustituirla por `NO_COMPRESSION` explícito en ese escenario.
...
```
SourceServer   : SRV-2K22-DB01
TestServer     : SRV-2K22-DB01
...
### Validación automática con dbatools y SQL Agent

La validación periódica de backups puede automatizarse mediante un job de SQL Agent que ejecute `Test-DbaLastBackup` y notifique por correo si alguna base de datos no supera la prueba de restauración o el DBCC.

#### Por qué CmdExec en lugar del subsistema PowerShell nativo

SQL Agent dispone de un subsistema propio de tipo `PowerShell`, sin embargo internamente utiliza `sqlps` — un host de PowerShell antiguo y restringido que puede provocar que el job quede bloqueado indefinidamente al intentar cargar módulos externos como dbatools. Usando el subsistema `CmdExec` invocamos directamente `powershell.exe` del sistema, que es el entorno estándar donde dbatools funciona con normalidad.

#### Script PowerShell — `H:\SCRIPTS\Test-LastBackup.ps1`

El script se guarda en un fichero independiente para evitar problemas de saltos de línea y comillas anidadas dentro del campo `@command` del job, y para facilitar su mantenimiento sin necesidad de modificar el job cada vez que se actualice la lógica.
```powershell
Import-Module dbatools

$resultado = Test-DbaLastBackup -SqlInstance SRV-2K22-DB01 -Database Test -EnableException
...
> ⚠️ El parámetro `@body` se pasa mediante `-SqlParameters` en lugar de interpolarse directamente en el string SQL. Esto evita errores de sintaxis cuando el contenido del cuerpo del mensaje incluye saltos de línea, comillas u otros caracteres especiales.
...
```
backupmediaset   ←──  backupset  ──→  backupfile
      ↓                                    ↓
backupmediafamily               backupfilegroup
```
